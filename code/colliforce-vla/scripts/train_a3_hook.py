"""
A3: SDF Gradient-Guided Action Refinement via openpi train_loop hooks.

Extends A1 (SDF auxiliary task) with gradient-guided action refinement:
uses ∂SDF/∂(suffix features) to compute a "collision-safe" action target
and trains the model to output it directly.

Two-phase training:
  Phase 1 (0 → warmup_phase_steps): Freeze backbone + LoRA. Train SDF decoder +
      EE extractor only (~658K params). SDF must be accurate before refinement.
  Phase 2 (warmup_phase_steps → end): Unfreeze LoRA (~33.5M). Enable gradient
      refinement. Train with full loss:
      Loss = action_loss + λ_sdf × sdf_loss + λ_ee × ee_loss + λ_refine × refine_loss

Key difference from A1:
  - A1: SDF is an auxiliary loss that improves feature quality
  - A3: SDF gradient explicitly shifts action predictions away from collision
...
Dependencies:
  ...
  - colliforce/sdf_gradient_refiner.py (SDFGradientRefiner)
  - SDF GT npz (query_points + sdf_values)

Usage:
    CUDA_VISIBLE_DEVICES=3 python scripts/train_a3_hook.py --scene spatial_bowl --dry_run
    CUDA_VISIBLE_DEVICES=3 python scripts/train_a3_hook.py --scene spatial_bowl --seed 0
"""
import argparse
import logging
import os
...
from colliforce.sdf_gradient_refiner import SDFGradientRefiner
...
# ---------------------------------------------------------------------------
# Manual LoRA — identical to A1 (PEFT not installed)
# ---------------------------------------------------------------------------
...
def inject_lora(model: nn.Module, pali_rank: int = 16, pali_alpha: float = 16.0,
                expert_rank: int = 32, expert_alpha: float = 32.0):
    """Inject LoRA with per-module rank/alpha matching baseline openpi config:
    PaliGemma (Gemma 2B) -> rank=16, alpha=16; Action Expert (Gemma 300M) -> rank=32, alpha=32.
...
# ---------------------------------------------------------------------------
# A3 SDF Gradient Refinement Hooks
# ---------------------------------------------------------------------------

class SdfGradRefineHooks(TrainHooks):
    """A3 hooks: A1 SDF auxiliary + gradient-guided action refinement.

    Phase 1: Train SDF decoder + EE extractor (aux only, same as A1).
    Phase 2: + LoRA + gradient refinement via SDFGradientRefiner.
    """
    ...
    def __init__(self, args):
        ...
        self.lambda_refine = args.lambda_refine
        self.n_query_points = args.n_query_points

        # Aux modules (shared with A1)
        ...
        # A3-specific: gradient refiner
        self.gradient_refiner = SDFGradientRefiner(
            alpha=args.refine_alpha,
            margin=args.refine_margin,
            max_refinement_steps=args.refine_steps,
            grad_clip=args.refine_grad_clip,
        )
    ...
    def on_model_created(self, model, device):
        ...
        self.gradient_refiner.to(device)
        ...
        # 4. Inject LoRA (initially frozen)
        self.lora_params, self._pali_lora_params, self._expert_lora_params = inject_lora(
        ...
        pali_lora_total = sum(p.numel() for p in self._pali_lora_params)
        expert_lora_total = sum(p.numel() for p in self._expert_lora_params)
        lora_total = pali_lora_total + expert_lora_total
        ...
        logging.info("=== A3 SDF Gradient Refinement Hooks ===")
        logging.info("  Backbone params (incl. LoRA weights): %.2fM", backbone_total / 1e6)
        logging.info("  LoRA PaliGemma (rank=16): %.2fM", pali_lora_total / 1e6)
        logging.info("  LoRA Action Expert (rank=32): %.2fM", expert_lora_total / 1e6)
        logging.info("  LoRA total (frozen for Phase 1): %.2fM", lora_total / 1e6)
        ...
        logging.info("  Gradient refiner: alpha=%.3f, margin=%.4f, steps=%d",
                     self.args.refine_alpha, self.args.refine_margin, self.args.refine_steps)
    ...
    def on_step_start(self, global_step, model):
        if global_step >= self.warmup_phase_steps and self.phase == 1:
            ...
            logging.info("  LoRA grad warmup: %d steps (via register_hook)", self._lora_warmup_steps)
            logging.info("  Gradient refinement now ACTIVE (λ_refine=%.3f)", self.lambda_refine)
            _m = model.module if hasattr(model, "module") else model
        ...
        # Phase 2 LoRA warmup: linearly ramp gradient scale over first N steps
    ...
    def on_loss_computed(self, action_loss, model, observation, global_step):
        ...
        # --- A3: Gradient-guided refinement (Phase 2 only) ---
        if self.phase == 2:
            _m = model.module if hasattr(model, "module") else model
            refine_loss, refine_info = self.gradient_refiner(
                features, self.sdf_decoder, self.ee_extractor, _m.action_out_proj,
            )
            if refine_loss is not None:
                total = total + self.lambda_refine * refine_loss
            aux_info.update({f"refine/{k}": v for k, v in refine_info.items()})
...
def parse_args():
    p = argparse.ArgumentParser(description="A3: SDF gradient-guided action refinement via hooks")
    ...
    # A3-specific: gradient refinement
    p.add_argument("--lambda_refine", type=float, default=0.1,
                   help="Weight for gradient refinement loss")
    p.add_argument("--refine_alpha", type=float, default=0.1,
                   help="Step size for SDF gradient shift on suffix features")
    p.add_argument("--refine_margin", type=float, default=0.01,
                   help="SDF threshold below which refinement activates")
    p.add_argument("--refine_steps", type=int, default=1,
                   help="Number of iterative refinement steps")
    p.add_argument("--refine_grad_clip", type=float, default=1.0,
                   help="Per-sample gradient norm cap for stability")
    return p.parse_args()
...
def build_config(args) -> _config.TrainConfig:
    ...
    exp_name = args.exp_name or f"a3_grad_refine_{args.scene}_seed{args.seed}"
    ...
    config = _config.TrainConfig(
        name=f"a3_grad_refine_{args.scene}",
...
def main():
    ...
    scene_cfg = SCENE_CONFIG[args.scene]
    logging.info("A3 Gradient Refinement (hooks) | Scene: %s | Seed: %d | Dry run: %s",
    ...
    logging.info("Refinement: lambda=%.3f, alpha=%.3f, margin=%.4f, steps=%d",
                 args.lambda_refine, args.refine_alpha, args.refine_margin, args.refine_steps)
