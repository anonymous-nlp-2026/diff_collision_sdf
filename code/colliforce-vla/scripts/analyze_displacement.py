...
Uses EE (end-effector) displacement to detect frozen episodes. Falls back to obstacle
displacement (max_displacement) if EE data is unavailable (pre-M044 eval JSONs)."""
...
def analyze_task(episodes, threshold):
    ...
    # Frozen = EE barely moved from start position (uses max_ee_displacement if available)
    has_ee = all("max_ee_displacement" in e for e in safe_eps) if safe_eps else False
    if not has_ee and safe_eps:
        print("[WARN] max_ee_displacement not found, falling back to obstacle max_displacement", file=sys.stderr)
    disp_key = "max_ee_displacement" if has_ee else "max_displacement"
    frozen_eps = [e for e in safe_eps if e[disp_key] < threshold]
